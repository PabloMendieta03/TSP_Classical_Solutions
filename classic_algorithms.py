...
class GeneticAlgorithm:
    def __init__(
        ...
        two_opt_rate: float = 0.02,
        generations: int = 500,
        tournament_size: int = 10
    ):
        """
        Enhanced GA for TSP with OX crossover, swap + 2-opt mutations, route validation,
        partial elitism replacement, and convergence monitoring.

        distance_matrix: symmetric np.array of distances
        population_size: number of individuals
        elite_size: number of best to carry over (5-10% recommended)
        mutation_rate: probability of swap mutation
        two_opt_rate: probability of 2-opt mutation per route
    ...
    def _is_valid_route(self, route):
        assert len(route) == self.N + 1, "Route length mismatch"
        interior = route[:-1]
        return set(interior) == set(range(self.N)) and route[-1] == route[0]

    def _create_route(self):
        route = np.arange(self.N, dtype=int)
        np.random.shuffle(route)
        route = np.append(route, route[0])
        assert self._is_valid_route(route)
        return route
    ...
    def _rank_routes(self, population):
        ...
        ranked = [population[i] for i in sorted_idx]
        return ranked, distances[sorted_idx]

    def _tournament_selection(self, ranked, distances):
        selected = ranked[:self.elite_size]
    ...
    def _breed_population(self, selected):
        ...
        for _ in range(self.pop_size - self.elite_size):
            ...
            child = self._ox_crossover(p1[:-1], p2[:-1])
    ...
    def _two_opt_mutation(self, route):
        if random.random() < self.two_opt_rate:
            ...
            assert self._is_valid_route(new_route)
            return new_route
        return route
    ...
    def _mutate_population(self, population):
        ...
        for route in population:
            r = self._swap_mutation(route.copy())
            r = self._two_opt_mutation(r)
            mutated.append(r)
        return mutated
    ...
    def run(self):
        ...
        for gen in range(1, self.generations + 1):
            ...
            std_cost = distances.std()
            if distances[0] < best_dist:
                best_dist = distances[0]
                best_route = ranked[0]

            # Log convergence stats
            print(f"Gen {gen}: best={distances[0]:.2f}, mean={mean_cost:.2f}, std={std_cost:.2f}")
            history.append((gen, distances[0], mean_cost, std_cost))

            selected = self._tournament_selection(ranked, distances)
            children = self._breed_population(selected)
            pop = self._mutate_population(children)

        elapsed = time.perf_counter() - start_time
        return best_route.tolist(), best_dist, elapsed, history
